package com.zq.flume.interceptor;
...
public class TimestampBodyInterceptor implements Interceptor {
	...
	public Event intercept(Event event) {
		...
		event.setBody(appendTimestampToBody(eventBody, System.currentTimeMillis()));
		return event;
	}

	/**
	 * Concatenates the body, a separator and the timestamp.
	 * 特别注意flume会自动在body最后添加  '\x0d' 回车标志，直接字符串拼接在body后注意处理这个
